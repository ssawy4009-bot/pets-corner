    ...
    filteredProducts.forEach(product => {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-info">
                ...
                ${priceSection}
                <button onclick="placeOrder('${product.name}', ${product.price || (product.variants ? product.variants[0].price : 0)})" class="btn btn-primary order-btn" style="width: 100%;">
...
// ===== API طلب المنتج =====
window.placeOrder = async function(productName, price) {
    const customerName = prompt("من فضلك أدخل اسمك لتأكيد الطلب:");
    ...
    try {
        ...
        if(response.ok) {
            alert("تم تسجيل طلبك بنجاح في النظام!\nسنتواصل معك قريباً.");
        } else {
            alert("حدث خطأ أثناء الطلب.");
        }
    } catch(e) {
        alert("تأكد من تشغيل السيرفر (Backend).");
    }
}
